...
-- get members from the latest year and month
SELECT firstname, surname 
FROM Members
WHERE strftime('%Y', joindate) = (SELECT max(strftime('%Y', joindate)) FROM Members)
	AND strftime('%m', joindate) = (SELECT max(strftime('%m', joindate)) FROM Members)
...
INNER JOIN Bookings b
	ON m.memid = b.memid AND b.facid = f.facid
